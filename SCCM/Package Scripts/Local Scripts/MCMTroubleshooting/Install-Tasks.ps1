<#
.SYNOPSIS
    Creates a custom event trigger and scheduled task for removing the MCM / SCCM client
.DESCRIPTION
    This script:
...
.NOTES
	...
	Write-EventLog -LogName 'USS-EventLog' -Source 'Remove-MCMClient' -EventID 1000 -EntryType Information -Message 'Trigger Remove-MCMClient with code'
#>
...
$LogDir = "C:\USS\Logs\Packages\MCMTroubleshooting"
$ScriptPath = "C:\USS\Scripts\Admin\MCMTroubleshooting\Remove-MCMClient.ps1"
$CodeFile = 'C:\TEMP\SCCMRemovalCode.txt'
...
$EventsAndTasks = @(
	@{
		EventSource = "Remove-MCMClient"		
		EventID = 1000									
		TaskName = "USS-Remove-MCMClient"
		TaskDescription = "Uninstalls the SCCM client if given the correct code. Source: MCMTroubleshooting package. Version: 1.0"
		Execute = "powershell.exe"
		Argument = "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$ScriptPath`" -CodeFile `"$CodeFile`""
		#WorkingDirectory = ""
	}
)
